...
void render_offscreen(bool first, TCOD_key_t*key, TCOD_mouse_t *mouse) {
        ...
        if (! init ) {
                init=true;
                secondary.printFrame(0,0,SAMPLE_SCREEN_WIDTH/2,SAMPLE_SCREEN_HEIGHT,false,TCOD_BKGND_SET,"Offscreen
console"); secondary.printRectEx(SAMPLE_SCREEN_WIDTH/4,2,SAMPLE_SCREEN_WIDTH-2,SAMPLE_SCREEN_HEIGHT,
                        TCOD_BKGND_NONE,TCOD_CENTER,"You can render to an offscreen console and blit in on another one,
simulating alpha transparency.");
        }
        if ( first ) {
...
random room size %s", bspDepth,minRoomSize, randomRoom ? "ON" : "OFF"); if ( randomRoom ) sampleConsole.print(1,6,"2 :
room walls %s", roomWalls ? "ON" : "OFF"	);
        // render the level
