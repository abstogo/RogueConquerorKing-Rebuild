...
bool CharacterManager::TurnHandler(int entityID, double time) {
  ...
  if (pcCurrentBehaviour[entityID] == -1) {
    ...
    // A switch, I hear you cry? Yeah, yeah, I know, not very DOD of me. If I was writing this to manage 2 million
    // enemies I might take it the other way (Actually that sounds like a bloody amazing idea if there's some way to
    // make them visible in a Roguelike. Maybe a battle sim where every unit is calculated?) But in all seriousness
    // there's not a lot of call to split the timing table n ways for existence-based processing if we only call this at
    // each timing point, and it's complicated enough
  ...
  if (pickNew) {
    ...
    std::string bhname =
        getCharacterName(entityID) + " sets behaviour:" + CharBehaviourNames[pcCurrentBehaviour[entityID]] + ".";
...
bool CharacterManager::TargetHandler(int entityID, int returnCode) { return true; }
...
int CharacterManager::AbilityBonus(int characteristicValue) { return AbilityBonuses[characteristicValue]; }
...
int CharacterManager::GenerateFighter(std::string name) { return GenerateTestCharacter(name, "Fighter"); }
...
void CharacterManager::SetBehaviour(int entityID, int behaviourType) { pcCurrentBehaviour[entityID] = behaviourType; }
...
int CharacterManager::EquipWeapon(int characterID, int itemID) {
  ...
  // with all weapons, shields, armour etc we need to check that we have proficiency with them and can use the
  // appropriate style.
  ...
  if (one_h) {
    ...
    if (pcEquipped[characterID][HAND_OFF] != -1) {
      ...
      // if it's a shield and we can sword & board, or if it's a weapon and we can pair, then equip the weapon
      // one-handed in our main hand.
...
bool CharacterManager::CanUseItem(int characterID, int itemID) {
  ...
  if (is_weapon) {
    ...
    if (std::find_if(
            pcWeaponProficiencies[characterID].begin(),
            pcWeaponProficiencies[characterID].end(),
            [itemID](const std::string& prof) -> bool { return gGame->mItemManager->hasTag(itemID, prof); }) !=
        pcWeaponProficiencies[characterID].end()) {
      // match found!
      return true;
    } else {
    ...
    if (is_armour) {
      if (std::find_if(
              pcArmourProficiencies[characterID].begin(),
              pcArmourProficiencies[characterID].end(),
              [itemID](const std::string& prof) -> bool { return gGame->mItemManager->hasTag(itemID, prof); }) ==
          pcArmourProficiencies[characterID].end()) {
        // match found!
        return true;
...
int CharacterManager::UpdateCurrentAttackValue(int characterID, bool missile) {
  ...
  int output =
      attack_bonus -
      tagValue;  // why minus? Because our attack values are roll-above, eg 7 gives "7+". So a bonus of 2 gives "5+"
...
int CharacterManager::GetCurrentSpeed(int characterID) {
  ...
  // encumbrance load can be 0-1/2 Base (Minimal, full speed), 1/2 - 3/4 Base (Light), 3/4 - Base (Medium), Base -
  // 2xBase (Heavy) Exploration Speeds are 120/90/60/30 for each category
...
std::string CharacterManager::GetCurrentEncumbranceType(int characterID) {
  ...
  DebugLog(
      this->getCharacterName(characterID) + " recalculated current encumbrance class as " + thing[encumbranceClass]);
...
int CharacterManager::GetCurrentAC(int characterID) {
  ...
  if (armourID != -1) {
    ...
    auto ac_tag = std::find_if(tags.begin(), tags.end(), [](std::string s) -> bool {
      const std::string pre = "AC|";
...
bool CharacterManager::getCharacterHasCondition(int id, int condition) {
  auto p = std::find_if(pcConditions[id].begin(), pcConditions[id].end(), [&](std::pair<int, int> t_cond) {
    return t_cond.first == condition;
  });
...
int CharacterManager::RemoveCondition(int id, int condition) {
  std::vector<std::pair<int, int>>::iterator iter =
      std::find_if(pcConditions[id].begin(), pcConditions[id].end(), [&](std::pair<int, int> t_cond) {
        return t_cond.first == condition;
      });
  if (iter != pcConditions[id].end()) {
    DebugLog(
        this->getCharacterName(id) + " removing condition " + gGame->mConditionManager->GetNameFromIndex(condition));
...
void CharacterManager::DebugLog(std::string message) { gLog->Log("CharacterManager", message); }
        ...
        if (!gGame->mMapManager->isOutOfBounds(mapID, new_x, new_y)) {
          if (currentMap->map->isWalkable(new_x, new_y)) {
            if (currentMap->getCharacterAt(new_x, new_y) == -1 && currentMap->getMobAt(new_x, new_y) == -1) {
              ...
              gLog->Log(
                  "Character Manager",
                  "Spawned " + getCharacterName(entityID) + " onto map #" + std::to_string(mapID) + " at position (" +
      ...
      if (new_x >= 0 && new_y >= 0) {
        ...
        if (baseCharacter != -1) {
          ...
          if (mobId != -1) {
            ...
            if (oldX > -1 && oldY > -1) m->setCharacter(oldX, oldY, 0);
...
std::vector<MortalEffect*> CharacterManager::GetMortalEffects(int id) { return pcMortalWounds[id]; }

void CharacterManager::AddMortalEffect(int id, MortalEffect* effect) {
  // check to see if we already have this Mortal Effect. If we have it already, we don't add it, but we check DoubleTo
  // to see if it upgrades
  bool found = false;
  ...
  // because we've added a Mortal Wound, we must update the Capabilities and Tag Cache to reflect the changes to the
  // character
...
void CharacterManager::DeactivateCharacter(int characterID) {
  // when a character dies, we don't delete it from the lists as that would produce all manner of weird effects and is
  // unnecessary. instead, we unset it from the map and remove all capabilities. This will prevent the character from
  // "ticking" etc and retain the character info for future use (eg RL&L, stored XP, memoriam screens etc)
...
std::vector<int> CharacterManager::GetConditionCharactersOnMap(int mapID, std::string condition, bool value) {
  ...
  while ((it = std::find_if(it, input.end(), [=](int c) { return getCharacterHasCondition(c, condition) == value; })) !=
         input.end()) {
    output.push_back(*it);
    // ReSharper disable once CppDiscardedPostfixOperatorResult
    it++;
...
std::string CharacterManager::getCharacterDomainAction(int id) { return pcDomainAction[id]; }

void CharacterManager::setCharacterDomainAction(int id, std::string action) { pcDomainAction[id] = action; }
...
void CharacterManager::setCharacterTravelMode(int id, std::string mode) { pcTravelModes[id].insert(mode); }

void CharacterManager::unsetCharacterTravelMode(int id, std::string mode) { pcTravelModes[id].erase(mode); }
