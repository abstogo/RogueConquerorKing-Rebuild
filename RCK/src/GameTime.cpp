...
TimeManager::TimeManager() { masterTime = 0.0L; }
...
void TimeManager::SetEntityTime(int entityID, int manager, long double time) {
  ...
  for (; ent_iter != entities.end() && !(*ent_iter == entityID && *manager_iter == manager);
       ent_iter++, manager_iter++, time_iter++) {
  }
...
void TimeManager::DebugLog(std::string message) { gLog->Log("TimeManager", message); }
...
long double TimeManager::GetRunningTime() { return masterTime; }
