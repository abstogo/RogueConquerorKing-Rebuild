...
std::vector<int>& BaseManager::getAnimals(int baseID) { return gGame->mPartyManager->getAnimals(basePartyID[baseID]); }
...
bool BaseManager::IsAPC(int entityID, int baseID) { return gGame->mPartyManager->IsAPC(basePartyID[baseID], entityID); }
...
int BaseManager::GetInventoryID(int baseID) { return gGame->mPartyManager->GetInventoryID(basePartyID[baseID]); }

int BaseManager::GenerateCampAtLocation(int partyID, int basePosX, int basePosY) {
  DebugLog(
      "Creating a camp controlled by party #" + std::to_string(partyID) + " at (" + std::to_string(basePosX) + "," +
      std::to_string(basePosY) + ")");
...
void BaseManager::DebugLog(std::string message) { gLog->Log("Base Manager", message); }
...
bool BaseManager::TurnHandler(int baseID, double time) {
  ...
  std::copy_if(basePartyCharIDs.begin(), basePartyCharIDs.end(), bedResters.begin(), [](int c) {
    return (gGame->mCharacterManager->getCharacterDomainAction(c) == "BedRest");
  });
...
bool BaseManager::TimeHandler(int rounds, int turns, int hours, int days, int weeks, int months) { return false; }
    ...
    if (controlPane == PANE_BASE_INVENTORY) {
      ...
      if (key->vk == TCODK_UP) {
        ...
        if (focus == PANE_BASE_CHARACTERS) {
          ...
          hench ? gGame->mPartyManager->AddHenchman(visitingPartyID, charID)
                : gGame->mPartyManager->AddPlayerCharacter(visitingPartyID, charID);
...
void BaseManager::RenderBaseMenu(int baseID) {
  if (baseID == -1) {
    ...
    if (baseOwner != gGame->GetSelectedPartyID()) {
      ...
      // outer frame
      tcod::print_frame(
          g_console, {0, 0, SAMPLE_SCREEN_WIDTH / 2, SAMPLE_SCREEN_HEIGHT / 2}, "", &TCOD_white, &TCOD_black);
      ...
      if (controlPane < PANE_BASE_INVENTORY) {
        ...
        for (int i = 0; i < out_cs.size(); i++) {
          ...
          for (BaseTag bt : cal) {
            // we have 1 domain action at a time, so if we match on that, indicate
            if (bt.Tag() == currentDomainAction) name += " " + bt.Indicator();

            // we can have multiple domain travel modes, indicate them
            if (gGame->mCharacterManager->getCharacterTravelModeSet(charID, bt.Tag())) name += " " + bt.Indicator();
          }
        ...
        if (charID != -1) {
          ...
          for (int i = 0; i < bt_list.size(); i++) {
            ...
            if (t.Indicator() != "") outp += " (" + t.Indicator() + ")";
...
bool BaseManager::CharacterCanUseAction(int baseID, int characterID, int tag) {
  ...
  if (t.Requires().size() > 0) return false;
...
void BaseManager::RenderBaseMenu(int xpos, int ypos) {
  for (int i = 0; i < baseXPos.size(); i++) {
    if ((xpos == baseXPos[i]) && (ypos == baseYPos[i])) {
      if (gGame->GetSelectedPartyID() == basePartyID[i]) RenderBaseMenu(i);
    }
  }
}
