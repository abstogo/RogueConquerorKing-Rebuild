...
void MobManager::SetMobX(int entityID, int x) { mobXPos[entityID] = x; }

void MobManager::SetMobY(int entityID, int y) { mobYPos[entityID] = y; }

int MobManager::GetMobX(int entityID) { return mobXPos[entityID]; }

int MobManager::GetMobY(int entityID) { return mobYPos[entityID]; }
...
bool MobManager::TurnHandler(int entityID, double time) {
  ...
  if (currentBehaviour[entityID] == -1) {
    ...
    switch (currentBehaviour[entityID]) {
      ...
      case MOB_BEHAVIOUR_FOLLOW: {
        ...
        // We need to generate a new path if we can see the enemy or if we reached our previous destination. Restore the
        // pathing check iff we don't have sensory lock
      ...
      case MOB_BEHAVIOUR_SEEK_ENEMY: {
        ...
        if (targetID[entityID] == -1 && targetManager[entityID] == -1) {
          ...
          std::vector<int> characters =
              gGame->mCharacterManager->GetConditionCharactersOnMap(mapID, "Unconscious", false);
          std::vector<int> targets =
              gGame->mMapManager->filterByFOV(MANAGER_MOB, entityID, MANAGER_CHARACTER, characters);
  ...
  if (pickNew) {
    ...
    std::string bhname =
        Monsters_[entityID].GetName() + " sets behaviour:" + MobBehaviourNames[currentBehaviour[entityID]] + ".";
...
void MobManager::SetBehaviour(int entityID, int behaviourType) { currentBehaviour[entityID] = behaviourType; }
...
int MobManager::GetTemplateIndex(const std::string creatureName) { return creatureNameLookup[creatureName]; }
        ...
        if (!gGame->mMapManager->isOutOfBounds(mapID, new_x, new_y)) {
          if (currentMap->map->isWalkable(new_x, new_y)) {
            if (currentMap->getCharacterAt(new_x, new_y) == -1 && currentMap->getMobAt(new_x, new_y) == -1) {
              ...
              DebugLog(
                  "Spawned " + GetMonster(entityID).GetName() + " onto map #" + std::to_string(mapID) +
                  " at position (" + std::to_string(new_x) + "," + std::to_string(new_y) + ")");
            }
          }
        }
        ticker--;
...
int MobManager::GenerateMonster(int templateIndex, int mapID, int x, int y, bool hostile) {
  ...
  if (mapID != -1) SpawnOnMap(nextMonsterIndex, mapID, x, y);
...
bool MobManager::TargetHandler(int entityID, int returnCode) { return true; }

bool MobManager::TimeHandler(int rounds, int turns, int hours, int days, int weeks, int months) { return true; }
...
std::vector<int> MobManager::GetAllMonstersInRange(
    int mapID, int centerX, int centerY, int range, bool hostileOnly, bool liveOnly) {
...
void MobManager::DebugLog(std::string message) { gLog->Log("MobManager", message); }
