...
void InventoryManager::TransferInventory(int sourceinventoryID, int destinationInventoryID) {
  ...
  for (int i = 0; i < itemEntries[sourceinventoryID].size(); i++) {
    AddItemToInventory(
        destinationInventoryID, itemEntries[sourceinventoryID][i].first, itemEntries[sourceinventoryID][i].second);
  }
...
int InventoryManager::SelectPrimary() {
  ...
  if (targetMenuInventoryID == -1) {
    ...
    if (ownerManagers[sourceMenuInventoryID] == MANAGER_CHARACTER) {
      if (gGame->mCharacterManager->GetEquipSlotForInventoryItem(
              ownerEntities[sourceMenuInventoryID], sourceMenuPosition) != -1) {
    ...
    if (ownerManagers[sourceMenuInventoryID] == MANAGER_CHARACTER) {
      ...
      if (mapID != -1) {
        ...
        for (std::pair<int, int> g : items) {
          for (int i = 0; i < g.second; i++) map->addItem(x, y, g.first);
        }
      }
    }
  } else {
