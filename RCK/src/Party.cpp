...
int PartyManager::GetInventoryID(int partyID) { return partyInventoryIDs[partyID]; }
...
bool PartyManager::IsAPC(int partyID, int entityID) {
  return std::find(playerCharacters[partyID].begin(), playerCharacters[partyID].end(), entityID) !=
         playerCharacters[partyID].end();
}
...
void PartyManager::DebugLog(std::string message) { gLog->Log("Party Manager", message); }

bool PartyManager::TurnHandler(int entityID, double time) { return true; }

bool PartyManager::TimeHandler(int rounds, int turns, int hours, int days, int weeks, int months) { return true; }
