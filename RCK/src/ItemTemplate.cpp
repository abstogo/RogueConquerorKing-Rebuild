...
ItemManager* ItemManager::LoadItemTemplates() {
  ...
  ItemManager* output = new ItemManager(
      jsoncons::decode_json<TemplateSet>(is), jsoncons::decode_json<DecorationSet>(ds), rangeSet, rangePenalties);
...
void ItemManager::DebugLog(std::string message) { gLog->Log("ItemManager", message); }
