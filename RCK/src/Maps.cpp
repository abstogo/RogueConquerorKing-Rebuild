...
Map* MapManager::getMap(int index) { return mapStore[index]; }

RegionMap* MapManager::getRegionMap() { return regionMap; }

bool MapManager::TargetHandler(int entityID, int returnCode) { return true; }

bool MapManager::TimeHandler(int rounds, int turns, int hours, int days, int weeks, int months) { return true; }

// region map creation
void MapManager::createRegionMap() { regionMap = new RegionMap(); }
...
std::vector<int> MapManager::filterByFOV(
    int sourceManager, int sourceID, int targetManager, std::vector<int> targets, int range) {
  ...
  for (int target : targets) {
    ...
    if (fov) output.push_back(target);
  }
...
int MapManager::buildEmptyMap(int width, int height, int type) {
  bool outdoor = true;
  if (type == MAP_DUNGEON) outdoor = false;
    ...
    for (int x = (stepped) ? 1 : 0; x < map_width; x += 2) {
      ...
      for (int terrain_index = 0; terrain_index < terrains.size(); terrain_index++) {
        ...
        if (terrain_value == t.RegionMapSymbol().c_str()[0]) {
          regionMap->map->setProperties(
              cell_x,
              cell_y,
              true,
              true);  // most terrain types are traversable at slow speeds and view distance is to be variable
...
int MapManager::SpawnLocalMap(int x, int y) {
  ...
  // this is to be replaced with a JSON file associating local content elements with procgen functions (eg spread,
  // warren, clusters, splats)
...
void MapManager::renderRegionMap(int centroid_x, int centroid_y) {
  ...
  for (int y = 0; y < map_height; y++) {
    ...
    for (int x = 0; x < map_width; x++) {
      ...
      tcod::print(
          g_console, {render_x, render_y}, tt.RegionMapSymbol(), baseColor, TCOD_black, TCOD_LEFT, TCOD_BKGND_SET);
...
void MapManager::renderMap(int index, int centroid_x, int centroid_y) {
  ...
  for (int mob : map->mobs) {
    ...
    renderAtPosition(
        ...
        centroid_x,
        centroid_y,
        gGame->mMobManager->GetMobX(mob),
        gGame->mMobManager->GetMobY(mob),
        cs,
        baseColor);
  }
}

void MapManager::renderAtPosition(
    int mapIndex, int centroid_x, int centroid_y, int x, int y, char c, TCOD_ColorRGB foreground) {
  bool outdoor = true;
...
bool MapManager::TurnHandler(int entityID, double time) { return false; }
...
void MapManager::AddItem(int mapID, int x, int y, int itemID) { mapStore[mapID]->addItem(x, y, itemID); }
...
void MapManager::DebugLog(std::string message) { gLog->Log("MapManager", message); }
