...
// local content covers things like doors, furniture, walls, that sort of thing. (Basically everything except entities
// and items.)
...
enum Hex_Movement { HEX_RIGHTUP = 0, HEX_RIGHT, HEX_RIGHTDOWN, HEX_LEFTDOWN, HEX_LEFT, HEX_LEFTUP, HEX_MAX };
...
class TerrainType {
 ...
 public:
  TerrainType(
      ...
      const std::string& RegionMapSymbol,
      const int OverlandTravelMultiplier,
      const int EncounterProbability,
      const std::string& EncounterTable,
      const std::string& Generator,
      const std::vector<std::string>& Prefabs)
      : Name_(Name),
        RegionMapSymbol_(RegionMapSymbol),
        OverlandTravelMultiplier_(OverlandTravelMultiplier),
        EncounterProbability_(EncounterProbability),
        EncounterTable_(EncounterTable),
...
struct RegionMap {
  ...
  int getLocalMap(int x, int y) { return (localMap[y * width + x]); }

  void setLocalMap(int x, int y, int c) { localMap[y * width + x] = c; }

  int getTerrain(int x, int y) { return (terrain[y * width + x]); }

  void setTerrain(int x, int y, int c) { terrain[y * width + x] = c; }

  int getSite(int x, int y) { return (sites[y * width + x]); }

  void setSite(int x, int y, int c) { sites[y * width + x] = c; }

  int getBase(int x, int y) { return (bases[y * width + x]); }

  void setBase(int x, int y, int c) { bases[y * width + x] = c; }
};
...
struct Map {
  ...
  std::vector<int>
      characters;  // characters uncontrolled by the player, ref to CharacterManager, position is stored there
  ...
  void addItem(int x, int y, int item) { items[y * width + x].push(item); }

  std::stack<int>* getItems(int x, int y) { return &items[y * width + x]; }

  void setTransition(int x, int y, int target) { transition[y * width + x] = target; }

  int getTransition(int x, int y) { return transition[y * width + x]; }
};
...
class MapManager : public ITCODPathCallback {
 ...
 public:
  ...
  void renderAtPosition(
      int mapIndex, int centroid_x, int centroid_y, int x, int y, char c, TCOD_ColorRGB foreground = TCOD_lighter_grey);
  ...
  // movement multiplier (based on map level - higher level maps have larger sized cells, therefore take longer to move
  // through)
  ...
  std::vector<int> filterByFOV(
      int sourceManager, int sourceID, int targetManager, std::vector<int> targets, int range = 0);
