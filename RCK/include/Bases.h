...
JSONCONS_ALL_CTOR_GETTER_TRAITS(PurchasableSlot, Name, Cost)
...
JSONCONS_ALL_CTOR_GETTER_TRAITS(BaseTag, Tag, Type, Indicator, MenuText, Requires, Excludes)
...
JSONCONS_ALL_CTOR_GETTER_TRAITS(BaseType, Name, Buildable, Upkeep, Core, Options, Purchasable)
...
class BaseInfoSet {
 ...
 public:
  BaseInfoSet(const std::vector<BaseType>& BaseTypes, const std::vector<BaseTag>& Tags)
      : BaseTypes_(BaseTypes), Tags_(Tags) {}

  const std::vector<BaseType>& BaseTypes() const { return BaseTypes_; }
  const std::vector<BaseTag>& Tags() const { return Tags_; }
};
JSONCONS_ALL_CTOR_GETTER_TRAITS(BaseInfoSet, BaseTypes, Tags)
...
class BaseManager {
  ...
  // It is quite possible for a party to control more than one base of operations. Indeed at later levels this is
  // entirely expected. There are 3 particular kinds of Base: 1) Camps (Temporary bases created by a Party) 2)
  // Settlement Bases (Towns and Cities, in which the Party has a base of operations such as an Inn or a house etc) 3)
  // Strongholds (Structures constructed or conquered by a Party outside an existing Settlement) These differ largely by
  // capability, and as a result are detailed in a JSON file. Note that while some capabilities are limited by type (eg
  // Camps cannot provide true Bed Rest, do not count as Return to Civilisation for XP purposes and do not allow Living
  // Costs expenditure), other capabilities must be unlocked in some fashion (usually by paying for it such as Storage
  // or Workshops) One final element is that Camps will eventually form the basis of the player construction system,
  // which means we probably want some kind of map representation of the Camp besides the region map icon.

  // NB - This is not the same thing as Settlements. Settlements are a whole other issue and will be dealt with in
  // SettlementManager - although a Settlement can be the site of a Base. The Settlement Interface will handle that
  // transition.
  ...
  // In future we'll have more complex controls for managing characters between parties (sub-groups etc) to make big
  // camps easier to manage, Why multiple parties? Because for example, wounded characters might be left in camp to
  // recover (bed rest) with some henches to guard them while other characters rove around the wilderness. Likewise a
  // freshened party may leave treasure with the carts in the base while dungeoneering. In all these cases, party events
  // (eg daily encounter rolls in wilderness) will continue to happen to the base party.
  std::vector<int> basePartyID;
  // the Owner Party is the party which created this base. This is generally the player's Party, although this may
  // change in future.
 ...
 public:
  ...
  int GetBaseOwner(int baseID) { return ownerPartyID[baseID]; }
