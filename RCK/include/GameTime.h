 ...
 * Since classic Roguelikes work on a system of moves more driven by impulse-movements rather than strict turn taking,
 * concepts like Initiative do not make sense. This requires quite a lot of changes. Most notably, a lot of the system
 * is mediated by means of Initiative (and order of declaration - most notably the spellcasting and defensive movement
 * rules)
 *
 * In order to resolve this, I've decided to use a two-resource approach: Time and Fatigue.
 * Time is measured in Ticks, which represent movement impulses. Since the game defines a map square (containing a
 * single man-sized entity) as being 5ft, that means a single tick is the time taken to move 5ft for the fastest moving
 * speed creature in the game. Each action taken consumes both Ticks and Fatigue. All other entities take their actions
 * when Ticks are advanced. Fatigue can be primarily restored by resting (although we may add some resources elsewhere).
 * Initiative will serve to slightly reduce the amount of time taken by non-movement actions, and determine who goes
 * first where they occur at once, as well as perhaps allowing for reactions if an enemy performs an action while you're
 * undertaking one.
 *
 * This will extensively affect many Proficiencies, which will need to be rewritten for the new system. It will also
 * affect the timing of spellcasting and the operation of defensive movement.
 */
...
enum TIME_PERIOD { TIME_ROUND = 0, TIME_MINUTE, TIME_TURN, TIME_HOUR, TIME_DAY, TIME_WEEK, TIME_MONTH, TIME_YEAR };
...
class TimeManager {
  ...
  // We store a "round" function for each entity (the player one returns the control to the user). This completes the
  // current action and then decides on the entity's next move.
 ...
 public:
  ...
  static long double GetTimePeriodInSeconds(TIME_PERIOD period) { return time_periods[(int)period]; }
};
