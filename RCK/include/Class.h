...
// I've gone down the route of creating them distinctly for this RL. However, I've attempted to keep the data structures
// relatively flexible So we can feasibly add new classes based on the PH class generation system later.
//
...
class ACKSClass {
 ...
 public:
  ACKSClass(
      ...
      const std::string& LevelChart,
      const std::string& AttackProgression,
      const std::string& SaveProgression,
      const std::string& SpellProgression,
      const int HitDie,
      const std::vector<std::string>& PrimeRequisites,
      const std::vector<std::string>& ArmourProficiencies,
      ...
      : Name_(Name),
        ...
        LevelledChartColumns_(LevelledChartColumns),
        LevelledAbilities_(LevelledAbilities) {}
...
class ClassManager {
 ...
 public:
  ...
  AdvancementStore* GetAdvancementStore() { return advancementStore; }
};
