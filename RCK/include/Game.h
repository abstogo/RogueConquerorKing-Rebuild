...
enum GAME_MODE {
  ...
  GM_MENU,
  GM_TARGET,  // target mode, selects from specific set of target Points and triggers something when Enter is pressed
              // (see TARGET_MODE)
...
enum TARGET_MODE {
  ...
  // TARGET_NEAREST_X,			// nearest X allies/enemies (eg Sleep, Bless) (0: range, 1:ally/enemy flags, 2:
  // X) TARGET_SPHERE,			// floor location to target a sphere attack (eg Fireball) (0: range,
  // 1:ally/enemy flags, 2:radius) TARGET_CONE				// target cone around character (eg Burning
  // Hands) (0:range, 1:ally/enemy flags, 2:width at widest end)
};
...
class Game {
 ...
 public:
  ...
  // Since our players shouldn't need to shift between modes just to use menu/selector etc controls, we also separate
  // those controls as well as moves
  ...
  bool ResolveAttacks(
      ...
      int defenderManager,
      int defenderID,
      bool missile);  // if returns true we're finished so return to GM_MAIN
  bool ResolveAttack(
