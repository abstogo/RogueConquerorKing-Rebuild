...
class PartyManager {
  ...
  //
  // In the current version the Party operates primarily as a means of collecting the PCs and henches together, as we
  // currently have no military game. Later the Party will also manage the local mercenary set and any local specialist
  // hirelings. To begin with we only have 1 Party. Later we will have non-player Parties and subordinate Parties (eg
  // you send a Hench and some military to fight goblins) Note: There are currently no plans to have henches of henches
  // in the game for simplicity, but subordinate Parties will handle some of that.
  ...
  // std::vector<std::vector<int>> mercenaries; // non-playable characters, only present in Region parties and Camps,
  // referenced to TroopManager and gets instantiated into MobManager std::vector<std::vector<int>> subordinates; //
  // other parties controlled by this party, referenced to PartyManager

  // Inventory
  // The core problem with how to model the carrying ability of the party is that we don't want to try to model the
  // carrying ability of each individual mule, hench, cart etc as well as the PCs themselves. Not due to processing -
  // it's actually pretty low-end - but because it would be incredibly complicated for the player to micromanage.
  // Instead, animals and "porter" henches are subsumed into a Party Inventory and items can be switched between that
  // and the player inventories. When the party enters a Settlement, items are sold directly from the Party Inventory.

  std::vector<int> partyInventoryIDs;

  std::vector<int>
      totalCarryCapacity;  // in stone. Total of all porters and animals (which includes carts etc). We assume the
                           // non-porter Henches and the PCs do not carry items between dungeons.
 ...
 public:
  ...
  void GenerateHenchman(
      int partyID, std::string name);  // only generates Lvl 0 Normal Men. Generate more advanced Henches using
                                       // CharacterManager and add them with AddHenchman.
  ...
  // void SetPartyInventory(int partyID, std::vector<std::pair<int, int>>& newInventory) { partyInventory[partyID] =
  // newInventory; }

  int getNextPlayerCharacter(
      int partyID, int currentPC = -1);  // if currentPC is -1, get the first one. Does not return incapable PCs;
                                         // returns -1 if no incapable PCs left (which generally means game over)
