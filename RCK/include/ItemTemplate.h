...
// Equipment Tags & Bonuses: None. Object name: figurine. Material: bronze. Decoration string: shape|long-lost deity.
// Value: 2000. Weight Num: 1. Weight Denom: 16.
//
...
// Equipment Tag: Melee:HH. Bonus: Attack Bonus|-1. Material: iron. Object name: broadsword. Decoration string:
// inlay|gold. Value: 1300. Weight Num: 1. Weight Denom: 6. (This is a randomly generated scavenged weapon with the
// "off-balance" penalty from the scavenged equipment table, which is a value penalty of 20%. The gold inlay is also
// randomly generated, adding an additional value equal to 50% of base. 10 + 5 - 2 = 13gp.)
//
// We load nearly all of the base information for this generation system from the equipment.json file, with decoration
// coming from the decoration.json file
...
class MaterialType {
 ...
 public:
  MaterialType(
      ...
      const double& Chance,
      const double& ValueMultiplier,
      const std::vector<std::string>& MaterialTags)
      : Name_(Name), Chance_(Chance), ValueMultiplier_(ValueMultiplier), MaterialTags_(MaterialTags) {}
...
class ItemTemplate {
 ...
 public:
  ItemTemplate(
      ...
      const std::vector<std::string>& EquipmentTags,
      const std::vector<MaterialType>& MaterialTypes,
      const std::vector<std::string>& DecorationTypes,
      const int& Value,
      const int& WeightNum,
      const int& WeightDen)
      : Name_(Name),
        Visual_(Visual),
        EquipmentTags_(EquipmentTags),
        MaterialTypes_(MaterialTypes),
        DecorationTypes_(DecorationTypes),
        Value_(Value),
        WeightDen_(WeightDen),
        WeightNum_(WeightNum) {}
...
class DecorationMaterial {
 ...
 public:
  DecorationMaterial(const std::string& Name, const double& Chance, const double& ValueMultiplier)
      : Name_(Name), Chance_(Chance), ValueMultiplier_(ValueMultiplier) {}
...
class Decoration {
 ...
 public:
  Decoration(
      ...
      const int& ValueBase,
      const std::vector<DecorationMaterial>& MaterialTypes)
      : Tag_(Tag), Text_(Text), ValueBase_(ValueBase), MaterialTypes_(MaterialTypes) {}
...
enum RANGES { SHORT_RANGE = 0, MEDIUM_RANGE, LONG_RANGE, RANGE_MAX };
