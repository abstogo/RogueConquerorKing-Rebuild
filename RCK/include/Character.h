...
// The universal ones include Hit Points, Hit Dice (which is determined in a variety of ways - level for levelled
// PCs/NPCs, or HD for monsters), AC (determined by equipment for PCs/NPCs, or just set for monsters), attack rolls and
// saving throws (based on class), and experience/level
//
// Many of our statistics are based upon Open Game Content. To ensure we keep with the terms of the OGC license, we need
// to store these statistics in a user-accessible format. We do this using JSON files (for more complex structures) and
// CSV files (for straightforward tables of data such as market stats), and we load them using jsoncons.
...
class Statistic {
 ...
 public:
  Statistic(const std::string& name, const std::vector<std::string>& bonusTo) : name_(name), bonusTo_(bonusTo) {}
...
class CharacteristicData {
 ...
 public:
  CharacteristicData(const std::vector<Statistic>& statistics) : statistics_(statistics) {}
...
// PCs will need quite complicated behaviours to manage class elements, while Henchmen should mostly be defined by their
// Role. Both of these will be defined in the Party manager, so we will reference back to that to determine which
// behaviour we're going to enact
enum CharBehaviour {
...
const std::string CharBehaviourNames[] = {
    "Idle", "Wander", "Follow", "SeekEnemy", "SeekItem", "Range", "Flee", "Unconscious"};
...
const std::string CapabilityNames[] = {
    ...
    "MagicCraft:Weapon",
    "MagicCraft:Armour",
    "MagicCraft:Construct",
    "MagicCraft:Crossbreed",
    "MagicCraft:Undead",
    "TreatWounds",
    "NaturalHealing"};
...
class CharacterManager {
 ...
 public:
  ...
  int getCharacterAbilityBonus(int id, int characteristicIndex) {
    return AbilityBonus(getCharacterCharacteristic(id, characteristicIndex));
  }

  int getCharacterCharacteristic(int id, std::string characteristic) {
    return getCharacterCharacteristic(id, GetStatisticIndex(characteristic));
  }
  int getCharacterAbilityBonus(int id, std::string characteristic) {
    return getCharacterAbilityBonus(id, GetStatisticIndex(characteristic));
  }
  ...
  int GetStatisticIndex(std::string name) { return StatisticLookup[name]; }
  ...
  int GetItemInEquipSlot(int characterID, int slot) { return pcEquipped[characterID][slot]; }
  ...
  int GetInventory(int characterID) { return pcInventoryID[characterID]; }
