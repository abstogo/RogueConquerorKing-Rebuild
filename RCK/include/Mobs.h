...
// "Mobs" refers to creatures (in creatures.json) and to "mob" used as the generic group noun for creature groups (in
// mobs.json). Creature groups will be in here but are currently unimplemented
...
const std::string MobBehaviourNames[] = {
    "Idle", "Wander", "Follow", "SeekEnemy", "SeekPlayer", "SeekItem", "Range", "Flee", "Unconscious"};
...
class AttackType {
 ...
 public:
  AttackType(const std::string& Name, const int DamageDie, const int DamageBonus)
      : Name_(Name), DamageDie_(DamageDie), DamageBonus_(DamageBonus) {}
  AttackType() {}
...
JSONCONS_ALL_CTOR_GETTER_TRAITS(AttackType, Name, DamageDie, DamageBonus)
...
class EquipmentType {
 ...
 public:
  EquipmentType() {}
  EquipmentType(const std::vector<std::string>& Always, const std::vector<std::vector<std::string>>& OneOf)
      : Always_(Always), OneOf_(OneOf) {}

  const std::vector<std::string> Always() const { return Always_; }
  const std::vector<std::vector<std::string>> OneOf() const { return OneOf_; }
};
JSONCONS_ALL_CTOR_GETTER_TRAITS(EquipmentType, Always, OneOf)
...
class CreatureTemplate {
 ...
 public:
  CreatureTemplate(
      ...
      const int HitDie,
      const int HitDieModifier,
      const int ArmourClass,
      const int Morale,
      const int Movement,
      const int XP,
      const double LairProbability,
      const std::vector<AttackType>& Attacks,
      const std::vector<std::string>& Abilities,
      const std::vector<std::string>& Behaviours,
      const std::vector<std::vector<std::string>>& AttackSequences,
      const EquipmentType& EquipmentSelection)
      : Name_(Name),
        Visual_(Visual),
        SaveAs_(SaveAs),
        Alignment_(Alignment),
        HitDie_(HitDie),
        HitDieModifier_(HitDieModifier),
        ArmourClass_(ArmourClass),
        Morale_(Morale),
        Movement_(Movement),
        XP_(XP),
        LairProbability_(LairProbability),
        Attacks_(Attacks),
        Abilities_(Abilities),
        Behaviours_(Behaviours),
        AttackSequences_(AttackSequences),
        EquipmentSelection_(EquipmentSelection) {}
...
class MobManager {
 ...
 public:
  ...
  // utility generators
  int GenerateMonster(std::string templateName, int mapID, int x, int y, bool hostile = true) {
    return GenerateMonster(GetTemplateIndex(templateName), mapID, x, y, hostile);
  }
  ...
  std::vector<int> GetAllMonstersInRange(
      int mapID, int centerX, int centerY, int range, bool hostileOnly = false, bool liveOnly = true);
  ...
  bool TargetHandler(
      int entityID, int returnCode);  // disambiguation: targeting system in the UI, not our pathing target
