...
class InventoryManager {
  ...
  // As a data element, we only stack identical items (note that similar items crafted apart from one another are *not*
  // considered identical, unless something special like Forgery is happening). However we can allow for similar items
  // to be collapsed together for display purposes.
  ...
  // You can also open an Inventory screen with no "destination" pane, eg the Inventory key, in order to manage the
  // current player entity's inventory (character or party).

  // Once we have a unified inventory system, we can then add Inventories to anything - which makes containers easy to
  // implement.
